/* sd2iec - SD/MMC to Commodore serial bus interface/controller
   ...
   ds3231.c: RTC support for DS3231 chips
...
#define STATUS_OSF     0x80  // oscillator stopped
...
void read_rtc(struct tm *time) {
  ...
  if (i2c_read_registers(DS3231_ADDR, REG_SECOND, 7, &tmp))
    return;

  time->tm_sec  = bcd2int(tmp[REG_SECOND]);
...
void set_rtc(struct tm *time) {
  ...
  i2c_write_registers(DS3231_ADDR, REG_SECOND, 7, tmp);
  i2c_write_register(DS3231_ADDR, REG_CONTROL, 0);   // enable oscillator on battery, interrupts off
  i2c_write_register(DS3231_ADDR, REG_CTLSTATUS, 0); // clear "oscillator stopped" flag
  rtc_state = RTC_OK;
}
...
void rtc_init(void) {
  ...
  tmp = i2c_read_register(DS3231_ADDR, REG_CTLSTATUS);
  if (tmp < 0) {
